...
for i = 1:5
    ...
    try
        load(['data_healthy/PeL_per_sub1_Session2_Trial' num2str((i-1)*5+1) '.mat']);
    catch
        load(['data_healthy/PeL_per_sub1_Session2_Trial0' num2str((i-1)*5+1) '.mat']);
    end
    PeR_temp = zeros(3, N);
    PeR = PeL_per;
    PeR = PeR';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
expData(1,:) = linspace(min(Data(1,:)), max(Data(1,:)), N);
[expData(2:nbVar,:), expSigma] = GMR(Priors, Mu, Sigma,  expData(1,:), [1], [2:nbVar]);

%% Plot of the data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure('position',[10,10,1000,800],'name','Healthy Elbow Position GMR');
%plot 1D
for n=1:nbVar-1
